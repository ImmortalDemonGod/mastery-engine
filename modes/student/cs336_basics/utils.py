...
def softmax(in_features: Float[torch.Tensor, " ..."], dim: int) -> Float[torch.Tensor, " ..."]:
    ...
    """
    # Test generic system with unique variable names
    my_tensor = in_features.float()
    max_value = my_tensor.max(dim=dim, keepdim=True).values
    shifted_tensor = my_tensor - max_value
    exp_values = torch.exp(shifted_tensor)
    return (exp_values / exp_values.sum(dim=dim, keepdim=True)).to(in_features.dtype)
