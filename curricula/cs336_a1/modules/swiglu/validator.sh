#!/bin/bash
# Validator for swiglu module
# Runs pytest for SwiGLU implementation in shadow worktree
...
if [ -n "$MASTERY_PYTHON" ]; then
    ...
    "$MASTERY_PYTHON" -m pytest tests/test_model.py::test_swiglu -v --tb=short --import-mode=importlib
elif [ -n "$VIRTUAL_ENV" ]; then
    # We're in an active virtual environment - use its Python explicitly
    export PYTHONPATH="$(pwd):$PYTHONPATH"
    "$VIRTUAL_ENV/bin/python" -m pytest tests/test_model.py::test_swiglu -v --tb=short --import-mode=importlib
else
    # No active environment - use uv to create one
    uv run pytest tests/test_model.py::test_swiglu -v --tb=short --import-mode=importlib
fi
