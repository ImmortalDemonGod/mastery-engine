#!/bin/bash
# Validator for bpe_tokenizer module
# Runs pytest for BPE tokenizer training implementation in shadow worktree
...
if [ "$(pwd)" != "$SHADOW_WORKTREE" ]; then
    ...
    cp cs336_basics/tokenizer.py "$SHADOW_WORKTREE/cs336_basics/tokenizer.py"
    cd "$SHADOW_WORKTREE"
else
...
if [ -n "$MASTERY_PYTHON" ]; then
    ...
    "$MASTERY_PYTHON" -m pytest tests/test_tokenizer.py::test_train_bpe -v --tb=short --import-mode=importlib
elif [ -n "$VIRTUAL_ENV" ]; then
    # We're in an active virtual environment - use its Python explicitly
    export PYTHONPATH="$(pwd):$PYTHONPATH"
    "$VIRTUAL_ENV/bin/python" -m pytest tests/test_tokenizer.py::test_train_bpe -v --tb=short --import-mode=importlib
else
    # No active environment - use uv to create one
    uv run pytest tests/test_tokenizer.py::test_train_bpe -v --tb=short --import-mode=importlib
fi
