#!/bin/bash
# Validator for text_generation module
# Runs pytest for text generation implementation in shadow worktree
...
if [ "$(pwd)" != "$SHADOW_WORKTREE" ]; then
    ...
    cp cs336_basics/generation.py "$SHADOW_WORKTREE/cs336_basics/generation.py"
    cd "$SHADOW_WORKTREE"
else
...
if [ -n "$MASTERY_PYTHON" ]; then
    ...
    "$MASTERY_PYTHON" -m pytest tests/test_generation.py::test_generate -v --tb=short --import-mode=importlib
elif [ -n "$VIRTUAL_ENV" ]; then
    # We're in an active virtual environment - use its Python explicitly
    export PYTHONPATH="$(pwd):$PYTHONPATH"
    "$VIRTUAL_ENV/bin/python" -m pytest tests/test_generation.py::test_generate -v --tb=short --import-mode=importlib
else
    # No active environment - use uv to create one
    uv run pytest tests/test_generation.py::test_generate -v --tb=short --import-mode=importlib
fi
